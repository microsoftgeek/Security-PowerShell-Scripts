...
try {
    ...
    $dateFrom = Get-Date $temp -Format "yyyyMMddHHmmss.0Z"
    ...
    foreach ($domain in $DomainNames) {
        ...
        $domainFailCount = 0
        $DN = Get-DN $domain
        $results = Search-Ad -dnsDomain $domain -attributes "msDS-ReplAttributeMetaData" -baseDN "CN=Policies,CN=System,$DN" -scope "OneLevel" -filter $filter
                    ...
                    if ($attrMetaData.pszAttributeName.ToLower() -Match $propertyFilter.ToLower()) {
                        if ($attrMetaData.ftimeLastOriginatingChange -gt $dateFrom -and $attrMetaData.dwVersion -gt 1) {
                            ...
                            $thisOutput = new-object psobject -Property @{
                                "GPO Name" = $result.DistinguishedName
    ...
    $res.Score = $score
    if ($score -ne 100) {
        $res.Remediation = "Due to their sensitive nature, ensure that any changes to these two GPOs go through well-known change processes and that any changes made to these GPOs are well-documented."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAp+poZoprH1Rzn
# F+CTvpfjzCC/kgav3OB+iMVOkOKiC6CCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIGCBUB/UScc9DZKhyIOPX0M1hbz6+Lr6t7RgbpGR
# gbGyMA0GCSqGSIb3DQEBAQUABIIBALOiWNXub4wcqeIkPV7ZdN8oJBHzOOJEttY3
# TbS/9eaQxDFNt72fmwfi33FwAYIaotapTPze31G7MogfXI97Rqs5+Oe2QJIb0quU
# 1QmS5+CKxSxD2FFlH4GucUNoZ282mtSJRlOBqUkmG5ecEn0xMXaJ0U7MBr48TXCy
# nN2ETu31Y+WGdmgyHicyxggqSjl2qxf7XPW4sQNIWq5q+mj7+Yhqj7utGDMCdfh/
# N31miglnymkeA3Qoyh/tg82TEqccQXeF1qMGP+i+XEqhyq721COipjFSZagv3Mx2
# HZ/fvJBTO1jBKpTP308qr0Fz7boSHrixFJ2X9wfuWAYks11yVpuhggIwMIICLAYJ
...
# AQkEMSIEIMq3llMr1oBXgYUvu1pGR7cYbteYJvvo/N5VX0n0UyWnMA0GCSqGSIb3
# DQEBAQUABIIBACV0raIFXaGz5C4C/wIjiUM/MMTKOseoWDds35SyAusn9v88GYqi
# 1bo/xj3zJZGy3wqV82Lc/AzI/dilNJ3fWT2GTB4yjBC07Kec3XRo9OvR4zqZSXo5
# wwP0E7POed+VYMRK5+STCyOvX/HXjm9WpjrN+GObVjZsEQwhfpI+aOZgvcCf4Ym+
# loTtKr4cE58sR2yu6yTHMbXpapKWHzRE4KJ1nu4qEQmutUDTstRWblVkew3hBmqW
# wnkeMG8AEsGrR9GsmjjybpGgzFS9bNfROnf2G7FJ0AfN4XPXghiP8aADhbhmvynf
# B7RO9KxTq4pvHkyiURnGq7FYuefz7dsnjKQ=
