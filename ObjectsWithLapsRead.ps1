...
try {
    ...
    $laps = Search-AD -dnsDomain $ForestName -attributes "schemaidguid" -baseDN $schemaDN -scope "Subtree" `
        -filter "(name=ms-mcs-admpwd)"
    if (!($laps))
    {
        throw "This indicator is not applicable as LAPS was not found in the forest."
    }
    ...
    foreach ($domain in $DomainNames) {
        ...
        foreach ($result in $results) {
            if (!($result.Attributes.'ntsecuritydescriptor'))
            {
                continue
            }
            ...
            foreach ($access in $securityDescriptor.Access) {
                # Check whether one of the access rights is what we are looking for
                ...
                # Only if the access is on all attributes or on ms-mcs-admpwd
                if ($access.ObjectType.Guid -eq $lapsGuid -or $access.ObjectType.Guid -eq "00000000-0000-0000-0000-000000000000") {
                    # Try to get identity SID
                    try {
    ...
    if ($failedDomainCount -gt 0) {
        ...
        $res.Remediation = "Ensure that there are no unnecssary principals who can read computer administrator account passwords via read permissions on the ms-Mcs-AdmPwd attribute."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAKrTJbGq2MekMB
# 7F1Nad9De5SEqrwhstewB5MNCMdvkqCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEID5Efo2TDEgx+p0JaBweUAnjRjw0vrDR2Ojg+F4b
# dCQpMA0GCSqGSIb3DQEBAQUABIIBAMqyu/oqQF+MRhx5j3hplmzGwlAMPWfoJWeS
# fbYdOYGXRnxi83Ur3Im76WHYZSgXXBx9AUZ36+kSWymQWZAppuBgUgUhVncTH5+U
# 8GNsq/eM4t2MXhivDB/Tg5y8X5UG25LCVKtEsb1Sucq90/0HgmopA7UT7MGDYsfS
# RbETSm9s7iu1b6/440cN3JDnOO6Y/ELZEqVlYm2HzVojqMTvRoBhHtsLlp+MxcV3
# P6ehqFGZ0pSIMetMwwUXDYVF1gc9xkAXOBnExLAhlTf49rr+e9osvUSJJyfUin84
# UHhaidlfb8/Ww1JSrATo1HwTIDfLPOI2iRg7AQx+qbxoRS9OtnKhggIwMIICLAYJ
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDMxNzE5MzM0NlowLwYJKoZIhvcN
# AQkEMSIEIOXQMqja2lmPiZJ2yhV3ACLD0cVH21eHSZsNIKh5ZdvkMA0GCSqGSIb3
# DQEBAQUABIIBADDsy/bXjNkZ3pp1XOp+gmclu5erOG7X+NLbXITLJtcjSzqdncrq
# oGl21HaM8QVzOIbpaJwz8bSLkcadcdbZABg/rBfkkznFUe1P1EKzXjq543ecnrsL
# J0wuDXNoQ+VpG7A5zKIvrxc7+z3qpFaZ7CikRv75lmh+r8Rd7MvI9pv6OZ+mCwk5
# hQ41MEw0KTCr6CNuM272FbMcMsirTkp4uPUmUs7Z48WohrFjaV1cb9Gfo5QQObeB
# B4dS+8x+xUTo2aqLtEQZy04Oi/o5mjeeM1rXHDB7b+wZXE0/9lJcsdLQj82+Ws5n
# +MJncl+WqZtzvuPZsBNdmGVqnMpbat8O4XU=
