...
try {
    foreach ($domain in $DomainNames) {
        $DN = Get-DN $domain
        $results = Search-AD -dnsDomain $domain -attributes "whenchanged", "whencreated", "managedby" -baseDN $DN -scope "Subtree" `
            -filter "(&(objectClass=computer)(servicePrincipalName=E3514235-4B06-11D1-AB04-00C04FC2DCD2*)(!(servicePrincipalName=ldap*)))"
        ...
        if ($results.Count -gt 0) {
            ...
            foreach ($result in $results) {
                ...
                $thisOutput = new-object psobject -Property @{
                    ...
                    CreatedDate = [datetime]::ParseExact($result.Attributes.'whencreated'[0],"yyyyMMddHHmmss.0Z",$null)
    ...
    if ($failcount -gt 0) {
        ...
        $res.Remediation = "If a host has been detected that has been used to launch Mimikatz DCShadow attacks, the host should be taken offline to prevent further compromise, and its logs reviewed to determine the attacking user."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC/o0xlMdAr4jh4
# m+o0mW/qzWbe85NALpyCDTW1O09vfKCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIFRtzlbrBPhMyP29yw5QmzRxMzfXXr/KvMfI8pLO
# k7gTMA0GCSqGSIb3DQEBAQUABIIBAE7MyuDv+wJDTxDY+vVVJ+07WLcMj0BVUpuz
# DL+IzIeOZZRVKTOZuHN4MT9MDZN4cYS54QnZr7LzP00Ma7S5td0lafwifiGT/G7J
# V2WaiyyNe5wtOnwFh/SHXQIrAEgUCs4GfYVQnU0Mn4XMkFy+bKXv0dOfPmxgUVeO
# owayXDAAb9J3QDZVkFPkcEwabbRrV3s+ukgDds4twdEqbNbx7UQa0j2ZpzM6j69F
# wK+fvyn/dleE2xqvCr2A3rXnBGlhAevqQUPACo0h4U++dDQ8+HE985sNW43e2saz
# 4tFg1ut0cwXQWfOwqr4ELleRC+0fsK4gWHQINDdCxQsM1BZ+EBOhggIwMIICLAYJ
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDMxNzE5MzMzOFowLwYJKoZIhvcN
# AQkEMSIEIJdjb34RAJIVwph3Vu0h3ny6CK6BEIEv2uBp3LPwYTRvMA0GCSqGSIb3
# DQEBAQUABIIBAAKWUYlWoOROAc1LbZP1JSWmr3hhyCVoo0jlOzKReXV3qSIuRDXe
# 1yBtazho+7ZtK0JY/U3bDNZGD/b4wJAcucbAtIcfDZ8WuDw4zoub/SLV11QcZSRS
# gHB6ZojXGW5i2L7aPmfZdSx5CduSpwAcuzGFxgxheuXx0wpNfTWrDv5c9m0iq1Dg
# dA/g7Nb+JyQlbDO4UlYX1+Snkr3yNQCt+C5mPyWT/2u69TitjwViMUG9GCz9+OoE
# QSXYfgXqTL5iqfejY6phWHlikKZfXaDtxj5rcf6xrursL96AtG4E4fs6mTfQwxHe
# nIa3VjeO82A94Xz9HTK4s2s+KQKk6nzaaRM=
