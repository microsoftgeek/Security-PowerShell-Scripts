...
try {
    ...
    if ($temp -lt $forestCreationDateTime) {
        $temp = $forestCreationDateTime
    }
    $dateFrom = Get-Date $temp -Format "yyyy-MM-ddTHH:mm:ssZ"
    $whenChanged = Get-Date $temp -Format "yyyyMMddHHmmss.0Z"
    $filter = "(&(whenChanged>=$whenChanged)(objectClass=classSchema))"

    # find any classes that have changed in last 90 days
    $results = Search-Ad -dnsDomain $ForestName -attributes "msDS-ReplAttributeMetaData" -baseDN "CN=Schema,CN=Configuration,$DN" -scope "Subtree" -filter $filter

    $propertyFilter = "defaultSecurityDescriptor"
    foreach ($result in $results) {
        foreach ($attr in $result.Attributes.'msds-replattributemetadata'.GetValues("string")) {
            ...
            if ($attrMetaData) {
                if ($attrMetaData.pszAttributeName.ToLower() -Match $propertyFilter.ToLower()) {
                    if ($attrMetaData.ftimeLastOriginatingChange -gt $dateFrom -and $attrMetaData.dwVersion -gt 1) {
                        $failedDomainCount++
    ...
    if ($failedDomainCount -gt 0) {
        ...
        $res.Score = 0
        $res.Remediation = "Confirm if any default security descriptor changes have occured to the detected objects in the last 90 days."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBq78zGkfo6Dj/D
# syFsg37A13RZaDfW9Ge7E1ph+BkQy6CCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIHsX1+4wgqilAUNpvJSwLl9rmCvIKQHQ3Vkzb4iP
# NXLtMA0GCSqGSIb3DQEBAQUABIIBAI74taqvZJGAYo54l3/22iU2RyGaAYM2Hq6w
# 4bct8U/jBcAauxmPk/gtWy31fJy+2C3cywZ4B/+pitqLWgCXi+v/2VwrQja1s6sP
# SGZwfXsNLsorf+mz7EZX3SS8tjDqUHEZex65+75oHdWS7zstwHu991NXkTdUCKtF
# wIVZcz1364mJ9pRjfkUt3G9hZzUkqpRwCZTmQB+UCyGlssbQBC2P1hY1DeUwQVwQ
# rkx09uDc+BexvD8rcq0HS+td2WVIsxnuFQd4SWnkHl4zn2KZI3y/E8+9+ZOSxmCj
# jN5BGfXrYLynBoVqEVUbEVtw98kdPFu4l5OBlqCrN81h42iWS3+hggIwMIICLAYJ
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDMxNzE5MzMzNlowLwYJKoZIhvcN
# AQkEMSIEIPOJZdNKvTiJ3y6a8mrDZfe2+5zH06OvVh+mxbRwkpLDMA0GCSqGSIb3
# DQEBAQUABIIBAEljC+ogQwN/xllD7FM2rapo02DEgEQ8lIwiItZ5lE7McWh77ooA
# hVuPFaj0K0hmMF1yFi6XAHB0A6sXSyZ4SefE7UTB0w+04NzMZwXGDIj7Nut06PZ2
# HAUzP2qctqowZJELeJtOAXQAc0o2ZZNOYl+qG6en7cn0o21JrPPDDZ6GujCeEHB8
# 8GfwaiXtTgV4GdjcYi8NX//vFPHeC7Y/WbD8yYWykdF1dEK+gje40zitDC/ndpaD
# YQtYIsXt/Sj3tDgSCqqR6LCsab74lORWESYqT4moiH/15aPR8rw5A4jbH0BUUGAZ
# dEcNQYoPwKt0sYxLpzo7cdmzKwprqIaL8E0=
