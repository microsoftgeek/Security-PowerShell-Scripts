# This script looks for computers that are memebers of protected groups
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all computers that are a member of privileged groups
        $results = Search-AD -dnsDomain $domain -attributes "memberof" -baseDN $DN -scope "Subtree" -filter $filter

        if ($results.Count) {
            $failedDomainCount++
            foreach ($result in $results) {
                if ($result.Attributes."memberof") {
                    $memberOf = $result.Attributes."memberof".GetValues("string") -join "; "
                }
                else {
                    $memberOf = $null
                }
    ...
    if ($failedDomainCount -gt 0){
        ...
        $res.ResultMessage = "Found $($outputObjects.Count) computers that are members of a privileged group."
        if ($DomainNames.Count -le 0) {
            throw [System.Exception] "Empty domain array"
        }
        $res.Score = 100 - (($failedDomainCount / $DomainNames.Count) * 100)
        $res.Remediation = "Check why those computer objects are members of privileged groups."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA8NQ+B0BM1xv09
# oN8Mi/0r72spsDhI4uU+WtAyt6je2aCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIHXp3rdElAOk/+G/19QKjdiWnk0y1Fnbqh93PIvx
# jWtLMA0GCSqGSIb3DQEBAQUABIIBAI9UReNsD2GNhaHYRuhfI/vraKPsUyOCx8+u
# 9QjQQUDcG7LT35rxsuAqXuNIvaTWyejrj1PPbZQe0uoDZ5elGu3LewHtotxI54Ms
# snqdwiSjb8DHJC0KBI3uR0KJhF+py8tqNTJdjBQXJ3BlHQne8M9pRtgnNzMbLuAR
# 319G9LUI8NrGdl1vU1Yi8qNGXCCYe5ea/D7uGb+J5IYVJ585AscVdYnNa9D4PLJF
# I0GlISW9396AKohGfVMipFkqG+h4jbrufmlmyKOipCNQX+y5SQGeG9KuYMPsYU0U
# G/l3UJhu7RilFdAgg7R/VvEuzQg0ICLdHJrrmGQYrZyQoiYH+RehggIwMIICLAYJ
...
# AQkEMSIEIOC9MlM7C9G26mfrvcTCTEPWEmFE3ddJ7kOoUu5lMmiKMA0GCSqGSIb3
# DQEBAQUABIIBABNV9Ia2bCtVwyC4Xr9ax6C3DSDWJKMTNkADq13lkFdmvziTSC7O
# CcNfoResFyWzebRxHrrCpPRa7M9NC2j5Rp3GZAgiFs/QSW3qrbgFkfZEDh9TQFZb
# 254xCeDrIH0Gn5VGJ3D+g7eyqlyYrTV7Ezsd9JjZfWvaIcQAcYquJUxuOLOhjc02
# t0ectDekN0dRBSb5JnaVsIcTLPB9aBBBwoXqBgmoRTB9HBOLVrbYRfZycb7sT1y0
# nCj4lhFlpkdJ2LCeq5xalHYjp8oiz9nbqxwe4IL/XGSd8c18OKc4gbG7tR3E4WAP
# 0dqE9bUvHLafKflP75V0PUHfx/Byn5JOesQ=
