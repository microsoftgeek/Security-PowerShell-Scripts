...
try {
    ...
    foreach ($domain in $DomainNames) {
        $DomainFailedCount = 0
        $DN = Get-DN $domain
        $attributes = @("dnshostname", "useraccountcontrol")
        ...
        foreach ($result in $results) {
            ...
            $dnsHostName = $result.Attributes."dnshostname"[0]
            ...
            if ((($userAccountControl -band 0x2000) -eq 0x2000) -Or (($userAccountControl -band 0x4000000) -eq 0x4000000)) {
                # search sitesContainer in config partition to see if a server object has the same dnsHostName
                $configDN = Get-DN $ForestName
                $resultsConfig = Search-AD -dnsDomain $forestName -attributes "serverreference" -baseDN "CN=Sites,CN=Configuration,$configDN" `
                    -scope "Subtree" -filter "(&(objectClass=server)(dnsHostName=$dnsHostName))"
                ...
                if ($resultsConfig.Count -eq 1) {
                    ...
                    # could not find a server object in config partition with matching dnsHostName
                    $DomainFailedCount++
                    $thisOutput = new-object psobject -Property @{
                        DistinguishedName = $distinguishedName
    ...
    if ($res.Score -ne 100) {
        ...
        $res.Remediation = 'Ensure that all DCs within your environment are healthy and functional. It is not uncommon for attackers to attempt to stand up "fake" DCs; however, these DCs are generally distinquishable from real DCs by the metadata found by this security indicator.'
    }
    else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCtLJ1jxgrhXJFZ
# fd/kUZkBzZaCPmJj7hxHlRbdSEjogqCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIB6RD/rtzGTbQ1+WoYYnPhwhqph6dIJ8JAzu1lq4
# usxoMA0GCSqGSIb3DQEBAQUABIIBAIUK+PSGsjZstQeqM4GI7IQ1FUPnFQOKewaX
# R0ph0MNXk300lod/5Zboy6czEKSydfuvrgijBpJvM060EtIESb3Zi/NnTgE52EAW
# AEGkjG8Qfm2J4eCqcYuaTycEdBBypFXcLkxiRKLxkHN9GXUX6V5DeTQUTdPzEphX
# 0hS5eOH5sVWeclGINdSUd14SpNoYENEtRGmcoqI+roTEWg6K5/r5Ou3P5rhEyvv9
# Ci14T3cMEbJHz1IAMoa3Q96rLntzPogNfpvcJZghD1W6+DaFrzOTD2VdFDcC2R/F
# 0Q7Rwx524QIDVY5ZfvMEWeNLM03W76+apa4q8FyDi5ULrK1XD0mhggIwMIICLAYJ
...
# AQkEMSIEIFZzLofDmsQh+sJcKzY639/c5UT4ZFXEUzDdvxszJA+qMA0GCSqGSIb3
# DQEBAQUABIIBAHtaAE+eDninZ0zHyByyMgmBPojdjl5p1MCB5pJp0rSlCJm4PBRb
# DOsMWCtAskzu+GFHFmGj4z5OAItAUQitFR2TCLZhUKnaIJsM5A4UubXH7jWJjIbV
# Nf4vpPC80iJNdgsY8c1m5WU6X4WfTQPMXED7z6nhDJRB51qRfIFsdUgRJfBdO01F
# UdFBaQ5CYpvAfpFGIM+mCMDQotnpdOuKI3GGZR6gB9jygLt72gwEPR3+SA6Y5Mwe
# exqVjWs6Q18QCLFL8J2+9C+cCfrczWh3ClIwqOCAeIEv1+p0evbW7Xo8dLjVSS8x
# fYvjsr8BUKjO/jhBIEBRQZjRF09flX7CiJg=
