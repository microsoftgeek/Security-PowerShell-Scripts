#Users with Risky User Account Control flags
#This script checks each domain to determine if there are any user accounts that have risk user account control flags set
Param(
...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $results = Search-AD -dnsDomain $domain -attributes $attributes -baseDN $DN -scope "Subtree" `
            -filter "(&(objectCategory=person)(objectClass=user)(!(UserAccountControl:1.2.840.113556.1.4.803:=2))(|(useraccountcontrol:1.2.840.113556.1.4.803:=544)(useraccountcontrol:1.2.840.113556.1.4.803:=524800)(useraccountcontrol:1.2.840.113556.1.4.803:=2097664)(useraccountcontrol:1.2.840.113556.1.4.803:=640)(useraccountcontrol:1.2.840.113556.1.4.803:=16777728)))"
        ...
        if ($results.Count -gt 0) {
            ...
            foreach ($result in $results) {
                ...
                $thisOutput = [pscustomobject][ordered] @{
                    ...
                    LastModified = [datetime]::ParseExact($result.Attributes.'whenchanged'[0],"yyyyMMddHHmmss.0Z",$null)
                    CreatedTime = [datetime]::ParseExact($result.Attributes.'whencreated'[0],"yyyyMMddHHmmss.0Z",$null)
                }
    ...
    if ($failcount -gt 0) {
        ...
        $res.ResultMessage = "Found $($outputObjects.count) users with a risky user account control."
        $res.Remediation = "These flags all represent potential weaknesses in user accounts, which if left in place, could make these accounts targets of takeover attacks. If these flags are required, ensure that these accounts have the least privileges possible and sufficiently complex passwords that are changed regularly."
    }
    else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAietlv8T9m54zZ
# HmqXvcDwHZ57l35nXrZkIglqXBHuMqCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIPhKZbifYSEnLzLVlD+3qED4bPyr/nVTXSgXQI8P
# FM2tMA0GCSqGSIb3DQEBAQUABIIBAKxkVd1HqSmGGvib0aRbDb3N/Z/eq1m7sCkR
# fPrHfiSpLHAM/lLRccGaH6roaM1f02rfppVVj3sHZ+jiUpp56Evgt/8Cvn9jy4DC
# IfvFNY6PZuy6FSUVwOJEBS0rPq2UaCZvZPkx+TAzupXL3EAUjBcYagsZKTU3YozC
# G8m6qSneZtmnloZmIh7hAy4Rs16EIXIEeOtNtDpwXlfOTrf77UafYWeM5/pHxeH1
# R6wmGk3ZlzMsiiwt37z6mp4OxpvsIRvr1ra+yPYMAo4pHd5YzpJXa4zivwlgVjzA
# 4oFaj2s2pkVPJVkRPDSjr9NlKAq37jg6JrTdVoW4NJh/4crHsFmhggIwMIICLAYJ
...
# AQkEMSIEILeJPg8/j3B9tjegEdfJUzM2CdYsptjtIZeiTERYsg1BMA0GCSqGSIb3
# DQEBAQUABIIBAEtfGXeDkXYmcIVCZrTX9txFhrklMqvVRd5BetgmiCMJuBxdNNeB
# WKznzfjgPMT5rgeE+NOA26cn4xjL6PylO12ST+zcqnouTf7R0p4QoK/uxvW4Y2b9
# 8atmIk4EkMLXWJyJ6elk+mgbvxR6Ndxzt1PLmnMqOBC08vqyTLKpbJFckmWFw5bG
# 7Dg/wYKYgGVO84Kemihnt+t9qe+ClbgEqDwokTzBL7LQFtn7esIC+V7GPnDKXsHu
# IS8gQp28LSQvT0QzmjzeikCSl7yJu6k5MdNCZpQPcvrf9ju/TDfugsa6Y8MCvGsu
# GJruaUPJgCVV5ZN2yEheQZzEFKwaA6sMPZ8=
