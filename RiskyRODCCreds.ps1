...
try {
    foreach ($domain in $DomainNames) {
        $DN = Get-DN $domain
        #Need to first get all the RODCs for the domain

        $results = Search-AD -dnsDomain $domain -attributes "primarygroupid", "msds-revealondemandgroup" -baseDN $DN `
            -scope "Subtree" -filter "(&(objectCategory=computer)(primaryGroupID=521))"

        if ($results.Count -gt 0) {
            foreach ($rodc in $results) {
                $allowedIDs = $rodc.Attributes."msds-revealondemandgroup".GetValues("string")
                # now iterate through each principal and member and see if it's an admin account
                foreach ($princ in $allowedIDs) {
                    # if it's a group, find it members and look for them being admincount = 1
                    $princDE = [ADSI]"LDAP://$princ"
                    if ($princDE.Properties["objectcategory"] -eq "Group") {
                        #first check to see if the group itself if privileged
                        $groupDN = $princDE.distinguishedName
                        $group = Search-AD -dnsDomain $domain -attributes "admincount" -baseDN $groupDN -scope "base" -filter "(admincount=1)"
                        if ($group.Count -gt 0) { # add to list
                            $failcount++
                            $scoreObjects += 0
                            if ($outputObjects.DistinguishedName -contains $group.DistinguishedName) {
                                # skip it since it's already in the list
                                continue
                            }
                        ...
                        $members = $princDE.Properties["member"]
                        foreach ($member in $members) {
                            $memberDE = Search-AD -dnsDomain $domain -attributes "admincount" -baseDN $member -scope "base" -filter "(admincount=1)"
                            if ($memberDE.count -gt 0) { #we have a hit so flag it
                                $failcount++
                                $scoreObjects += 0
                                if ($outputObjects.DistinguishedName -contains $memberDE.DistinguishedName) {
                                    #skip it since it's already in the list
                                    continue
                                }
                                $thisOutput = new-object psobject -Property @{
                                    DistinguishedName = $memberDE.DistinguishedName
                                }
                                $outputObjects += $thisOutput
                            }
                        }
                    }
                    else { #it's a user or computer so see if it's admincount = 1 {
                        $userDN = $princDE.distinguishedName
                        $user = Search-AD -dnsDomain $domain -attributes "admincount" -baseDN $userDN -scope "base" -filter "(admincount=1)"
                        if ($user.Count -gt 0) { # add to list
                            $failcount++
                            $scoreObjects += 0
                            if ($outputObjects.DistinguishedName -contains $user.DistinguishedName) {
                                # skip it since it's already in the list
                                continue
                            }
                            $thisOutput = new-object psobject -Property @{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCljQJqiLQm0Ans
# T4vyirBT2S+l0KYC2sdXNpc78Y7jwaCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIOoQ/UgPgBz2sXuQ5usgv/4uFTkGNcYX/43ruXN7
# Sk4OMA0GCSqGSIb3DQEBAQUABIIBADZLjQF22YejCGb09/xWZ1XOUgmGSI9wcrEu
# GX5w0ceUGuFsTeRAqSwnRuPNqeC+BsFKFukAK/B8sT1YZBFimImXm5WuLeI5oh0K
# mIDu/pw8pgbP2yjQfcghG+msmAra4Z2Z2EuymXY+x1wJ9TmP2X6zlDeZDqSBpo5e
# OyFK/Yj5QBaGq/EgB3xn0769I2c6KnA6N8gJ7l/WQX9tjBgsYw3T5EE96DLHm9RR
# zHiIlK4n7Qgr73I/+5RvZcwAt9jPFn8J24/B5egSZFqotdc1qdzm9Zk4//GEfnoZ
# mksiwEwyxe4uZ8zwJ8fT1nZx35AwNPYtCpXIGp0lh9LeR3E37YShggIwMIICLAYJ
...
# AQkEMSIEINgasCW4HMeBMbxuuISG6IkhmsLCAhAlJFs/VqyfpPTUMA0GCSqGSIb3
# DQEBAQUABIIBAHqfvjFqZD2uJbuCfOcBKPA6IVJGECSw/1RuKro8q4OTiRJtW7wF
# QeTPObaPQK1lF0ZYm8L/aj6ZxgWTstQxVFrDfRawoWrT9t4BYEx0/3qJEmjBlHB2
# AbQHb/TzGVBOTNsvriSxeE96YhJGvhlaN+ptSOTdd3W8NGfP/O12sjlmdYWbv1Ua
# 4Fho2+34ixIzGC5Ft302+jYxmtglfuIp067Hlo5aPNWyIB/jF+JgAKMIvKtDw17W
# Rz3SQ/ArNDPRAJzTR5jS5+DohkUlKDohp8xoLL0JY5/EpLO4Y2yUisFrl3yPKyV6
# oeH+0naT+4OcXCMYV/E2maOZNFfg62meSZY=
