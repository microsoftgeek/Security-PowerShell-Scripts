# This script looks for permissions changes on AdminSDHolder that occured in the last 6 months
...
try {
    $temp = (get-date).AddMonths(-6)
    $lastOriginatingChangeThreshold = get-date $temp -Format "yyyy-MM-ddTHH:mm:ssZ"

    foreach ($domain in $DomainNames) {
        $DN = Get-DN $domain

        $results = Search-AD -dnsDomain $domain -attributes "msDS-ReplAttributeMetaData" -baseDN "CN=AdminSDHolder,CN=System,$DN" -scope "Base" -filter "(name=AdminSDHolder)"
        ...
        if ($results.count -gt 0) {
            foreach ($attr in $results.Attributes.'msds-replattributemetadata'.GetValues("string")) {
                ...
                if ($attrMetaData) {
                    if ($attrMetaData.pszAttributeName -Match $propertyFilter) {
                        if ($attrMetaData.ftimeLastOriginatingChange -gt $lastOriginatingChangeThreshold -and $attrMetaData.dwVersion -gt 2) {
                            $failedDomainCount++
                            $thisOutput = new-object psobject -Property @{
                                DistinguishedName = $results.DistinguishedName
                                Attribute = $attrMetaData.pszAttributeName
    ...
    if ($failedDomainCount -gt 0) {
        ...
        $res.ResultMessage = "Found $failedDomainCount domains with AdminSDHolder container permission changes in the last 6 months."
        $res.Score = 100 - (($failedDomainCount / $DomainNames.Count) * 100)
        $res.Remediation = "Check the permissions on the AdminSDHolder container and search for abnormal ACE \ owner."
    }
    else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCZHaDAsF6tsLPr
# SWtrcfTyylFqWJk4nqZGMImCCBUl96CCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIPkbLNjdXj7FWZgTbIkht1lx5v11InqSWu8I3SkN
# ZbPWMA0GCSqGSIb3DQEBAQUABIIBAFJPI9GBlbyD9Caqro2sX0gWX17G1uti6iYP
# a2sYcUXQtx2/lPssYvmf8rF1s8vne6AkQPgQqNtbJvJ1MTagXMozhBZ8iWO4BSSg
# fvZbrxbpQol8jc/TtwYGY2BAdMElouXPkq+TRL9/ujOLx5mtKdeCOb+hYXB+OQDQ
# ByO8q9BOtKE0DjhYsBZQZn8ZSrof0IXbtCtHB6nPpPoG2heNxFZj3kNM41ROsoXf
# mZk2IaggEjUM/GJ3LRX576Hmb86/6cXcSc98TR2QFj+FNLmHQWqWtKv1sXWZsiwj
# SnzAtrONtIrm/Zvl+xloSIL8ZN+I5tMQy3jcqT4nQYmuZEN8AfehggIwMIICLAYJ
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDMxNzE5MzMzNFowLwYJKoZIhvcN
# AQkEMSIEIEa2KYCeKRNpTntFL/YpSHZbwsTt6IP04avp8zphxeHmMA0GCSqGSIb3
# DQEBAQUABIIBAEaXvecA4npe3zUsZ9b5GitnRvfd7Qq0enRiaB+aWAsAX3ppEgdG
# X7RNg22auIkhapo1lvDIhRqn7KnidsUOs4xDOLEVeJsTlcyJnvQjx+Dk845Sbh/B
# 2vQKSZSYa2GtOeV6A9C5DWV5MY5x6P+8INffJv6ZJXQFclCC0du8L9nOm8bsYZ+/
# Q+tkYgdr3Tu8ZwdY8em462SYJeRUYYYI2e+ujFRJk0YzlQ5+nM0JJ4nrw9EhB4dv
# 8vrhnu6D+qY7Cj0CzTZ8FS5pW7gjyOI3Nn/eRYO3sK8tWoNTyYgvsnNDou3Addlg
# bQ7jCxVjiTACzwFZQ7x4/pT0+Oc/CqEA/KQ=
