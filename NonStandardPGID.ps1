...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        $userFilter = "(&(objectClass=user)(objectCategory=person)(!(primaryGroupID=513))(!(primaryGroupID=514))(primaryGroupID=*))"
        ...
        $PGIDResult = Search-AD -dnsDomain $domain -attributes "primarygroupid" -baseDN $DN -scope "Subtree" `
        ...
        foreach ($result in $PGIDResult) {
            ...
            $group = Search-AD -dnsDomain $domain -attributes "member", "samaccountname" -baseDN $DN -scope "Subtree" `
            ...
            if ($group[0].Attributes."member") {
                if (!($group[0].Attributes."member".GetValues("string").contains($result.DistinguishedName))) {
                    $found = $true
                }
            }
            else {
    ...
    if ($failedDomainCount -gt 0) {
        ...
        $res.Remediation = "Change member's Primary Group ID or add the member to the relevant group."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCVxqapQ3XYTTvN
# d7xZfThg4QR2t4hiZ9zGiSTTBY6RV6CCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIIto2RZUYTXBPSYABgjLqCmt6wyDA4HT9194wTm9
# +ZRxMA0GCSqGSIb3DQEBAQUABIIBAJaFYHILxdoGOoRxmJW1g2jniDmiS2Z1qLSU
# ZUc9+bGlBmUIbOTHwY/M5+FLRr7RFsjfffaUMnZFW9wnDLd+fuyK8bkOg932g2jL
# pzY95amJg92BYzpp4VN0uK0+B73NgYLfDVUwy1xvGxKrTuGyXmJlxWrqCafT5HIi
# z4E8/tTA6FiaYsiUyeLniM1iSs+sxaaEnSh97YZzfVU40XitMyO90Ei+TEt/it1s
# uPj6lF/2a1MRryUfZq6DaJ71Nxi0WFcuEl2CknL3EksvwrviWtfXgD06MDpAx/ol
# hJe25erx3Rs2dtF65wJF2g9zc7rwtkCHfcpfBg/ig+SiFlyhRDChggIwMIICLAYJ
...
# AQkEMSIEIMpuXNxlhyXmSBmF7e8n1mQb6YRRSO8QDGzrTvc3J6+qMA0GCSqGSIb3
# DQEBAQUABIIBAJN0RNMSo8/dhhxfFF8uYUN9FDi45LVpM+7im7quKebg93UkKAiF
# AJ+sztrdWBmtQmHtB99ZydBF7097fWeePuc2Lx0o2S7/dDAng4y7pqQmLscY0yZ8
# SfLbxPohYSRDm+ZIw+1tjhqtZxVbGTRELUDDkhDLp8kc85yIQ2D0xnKerAzv3erG
# n8UIEcjkVwyOEQMsgAmYxmHXeIpGAR/+eUUjiX9+Vgv7YMLQ/8sX8OgzZm+JicIC
# 8raRppCvh/IWt1VTbKPQ9Tc+3xNx9giiy/cCILmesBGonAYRDh89K5ZP+/GStdQc
# uHnTOeqhqNhHSRLtFeRxuYS50SG3hFliKV8=
