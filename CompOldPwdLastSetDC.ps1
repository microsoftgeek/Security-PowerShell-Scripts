...
try {
    ...
    foreach ($domain in $DomainNames) {
        ...
        # Get a list of all enabled DCs that didn't changed their password in a while
        $results = Search-AD -dnsDomain $domain -attributes "pwdLastSet" -baseDN $DN -scope "Subtree" `
            -filter "(&(samaccounttype=805306369)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(pwdLastSet=0))(pwdLastSet<=$pwdLastSetThreshold)(primarygroupid=516))"
    ...
    if ($failedDomainCount -gt 0){
        ...
        $res.Remediation = "Ensure that any DCs found by this indicator have a valid trust relationship with the domain and are properly replicating to partner domain controllers. Use repadmin /showrepl to test replication and run nltest /sc_verify to check for a secure channel connection to the domain."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDcE/zzDcOPaYId
# SZbtVyZg4hpQdjSzxGWnWhSxBu++C6CCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIJ8OYsFHUbxGGnnO+K9KER6RF7MPIMHOXW9QYf2U
# qmIkMA0GCSqGSIb3DQEBAQUABIIBANDeYcmkOmivzYFj6jYb7JkAyS0qI985Nsvi
# LYQk0v6xkytODitOjs3IAnEItnXpVPRP5BXh+2ujPstbnYI0oexwVKQ2zI64pjT7
# yaTh4114fOUcCXrSrROR/lDaEvDcGwzuuOqE+cnfusMU+6vLK80Z94nJsOHuxmpx
# uSVLqqaR3LxqwhZlUYA/gJroKwILBU3PNcdGE8PsfVfYNw2UuB+/PGosD2gcSsFV
# Kz19fcbu3x/kAg0Ghys0z3BwFbJZMZEAhq70Iid3gHoB9nOoXhQ76+Z9iMZLZwRu
# 5b+IxU6ffzJZ8X8WK3T1tiDhnmmiRktSa8KUOU8ph3VXoP+E5ryhggIwMIICLAYJ
...
# AQkEMSIEII3JAd/2D40K32WUOVRjs8oxfL+e7zm7UCu0QzukFVT8MA0GCSqGSIb3
# DQEBAQUABIIBAHHK7gWOPA4kBa9nPv+hduTmHI9XEwFHDmR2V6RGfgp5kFJrsFOL
# D+Q4Sii6tZmQMZgdbiMpKcKLThP+/aV9rxpOeIq9VdyCZScouKyAMG4tXsrVnqBv
# 8UknH9uaLZ0vzQWNT+lSOdIhfeZUyKNuhmlV8iho+upxBbXLjHZAz1M0OpUicg59
# n5p9Rpo2Te5AF8IhMIX2wrsnsaVQhoZw9QEbkegjbYxJBUTekNvIecvA+EIeEpi2
# ZhewB/US2BlxCQmJv+VApAdgYEPZGNMXWIHDBFhSUb+IuiEl+9bVJsJEncbQFVFZ
# UbuNa8DYzqA4dhxHTdO4NyYRk6h/6uySRyw=
