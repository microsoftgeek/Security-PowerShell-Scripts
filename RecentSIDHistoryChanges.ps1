...
try {
    ...
    $monthsToRemove = 2
    $temp = (get-date).AddMonths(-$monthsToRemove)
    $lastOriginatingChangeThreshold = get-date $temp -Format "yyyy-MM-ddTHH:mm:ssZ"

    foreach ($domain in $DomainNames) {
        $domainFailsCount = 0
        $DN = Get-DN $domain
        $results = Search-AD -dnsDomain $domain -attributes "msds-replattributemetadata", "sidhistory" -baseDN $DN -scope "Subtree" `
            -filter "(sIDHistory=*)"
            ...
            foreach ($attr in $result.Attributes.'msds-replattributemetadata'.GetValues("string")) {
                ...
                if ($attrMetaData) {
                    if ($attrMetaData.pszAttributeName.ToLower() -Match $propertyFilter.ToLower()) {
                        if ($attrMetaData.ftimeLastOriginatingChange -gt $lastOriginatingChangeThreshold -and $attrMetaData.dwVersion -gt 1) {
                            $domainFailsCount++
                            $sidHistory = $result.Attributes."sidhistory".GetValues("string") -join "; "
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCH2gJ5RbBQobjI
# U6h9mISb+qRcbwKIgcKLYi4NsH9If6CCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIFwhEYSqb2h+T9BdHQukBAp/b7kHt6uos4I/5cgp
# 6P2kMA0GCSqGSIb3DQEBAQUABIIBAHyaeHqbzV+VsD5HlyOfsKTqfwFTH+r2uXeC
# U3c8dL2QG3PODUfcm8P3+zbfCeMv2h3zqMWIL6Yj4m2NUxEi5naTyWPHHfqBkx2C
# Ky+YaqbofkPs37cBODLwyw1kdE3qYNfAj0LTnr8W3Tm0dkC0h7GktIoQujm6Ogz6
# X+pwx6vVkshYiQlNRTP35sBrfKR4ydvdKFErQX1Vp7YbP8mCxEwBuKTkhsUWd+31
# eeWJEaOfx/cNcOfURHRmcJIRsMQmimuFByV3lsXyJ2eexogLMmYC3NBlVqynpE7T
# +Uem5nGp4kW+N3G1X+j9hKqTYQJ0VAX5u6BSTET856TVO/avV+ihggIwMIICLAYJ
...
# AQkEMSIEIB/sm3Yzz1/1vtdYJWEmUvOC6Ac96gCGYo7KSu8PiEbcMA0GCSqGSIb3
# DQEBAQUABIIBACRiYbpJt5c5/MpQqX237Ef6AFZJx9Ny+bLmu7cNkWrVpKM9Kk3J
# Rr+hVBw15HlmXd9tOd6o9VWLiGMfza+m/0jnBt707U08rIyAjaAF2KzOgzXlzRLH
# 5fL137uFx4xfxmKEGIK5e/hvdID+PiKctBAS1EIitBPUnP1xdNdM1kKhIJR3aguo
# XWM2xxlQxM/DdZFSjjkNOHd0sdrvVhkWgixsqS43V6lLNT9zNWxdXLUBx3TNLfah
# 2BKD8JSz14ttcK+F+LB+q1UL10UKtbZdIlmamU5eF15u2Z0XDBD21FXUfwPv8ihi
# oJOu4lw6eEnEYNo3HnXkqrCGJSgB+WpPUEg=
