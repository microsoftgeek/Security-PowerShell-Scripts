...
try {
        ...
        if ($UsersACL.Count -gt 0) {
            ...
            foreach($result in $UsersACL.GetEnumerator()) {
                $thisOutput = new-object psobject -Property @{
                    DistinguishedName = "OU=Domain Controllers,$DN"
    ...
    if($failedDomainCount -gt 0){
        ...
        $res.Remediation = "Unprivileged users should not be able to link GPOs at the Domain Controllers OU level. Doing so essentially gives them the ability to escalate their access, change domain-level security posture, and use GPOs to effect all systems and users in AD."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAs+f6MDTSELM8q
# sh1Qztvq9/IL/u+yh+o85C/Dq2+UzKCCE+UwggSUMIIDfKADAgECAg5IG2oHJtLo
...
# ARUwLwYJKoZIhvcNAQkEMSIEIJcy3eMtJnl+L3/qQPpHJMeH0zDofFENrAjL5skf
# H/fDMA0GCSqGSIb3DQEBAQUABIIBAJmqPGg+thJiLD5KQXr9AFaUc36OTGm4lkcL
# QVYMQBP0//yaErzk+JDMI48vAB1iCDDlKz4LYJq7MIXZbc9uB+3V2nzmQYnbE9Ur
# Omq1FgqZbJiIWmQIdov/TPGH116GSE/6ybCLutGEifc+bBGLn16vX+uDChgHcq6O
# gF0FiNti3YnAoriKyCV9lMeD+QNuYILWc7Ctt8F9uB06f/kClzbwEb23CWBpO88q
# 5IQe6W8ZRM+KYECrP/U772MvPPeZbVOko+3nvXwfY1Zgi8Juj5kiPfP1RylMtDhy
# Eh7a3jC73EKlQaWmTrp7DQtr++RJwOjZbxDTAMtpdH5Er7AXtHChggIwMIICLAYJ
...
# AQkEMSIEIHLcQPYpAMvElGzcRoDECVk02hE+M+GE/hkmp2B33d2aMA0GCSqGSIb3
# DQEBAQUABIIBAJZaggQOHXgOp16Q4/SY+S3mIpf+z6vhqwb89+rYCXVel1JsDmUm
# sjiuLDw+EtLiXHZtT/ch3gvlpw1x6ALSk8UULOSoCIY7Xv4T/4lEX6/bek9CDAKY
# XrsHR4338jtAYzzqQKGUtasauQ4xWjx8ixwFtxP4JXRJg75jkyjdTpxJa7fUJPwy
# 1H16sCqgABPzSrzL2/3ISsQWi8R7sUSYrky1vBSV2pLpyFyyD7Ob6oJKmRiEkRi9
# Puy629q1Qjq8ezFHkhudTTBLqPlWNgokw8E3p/QaBZgqrVSrL8wC6YFEb0NAMmNu
# 7tPTk3+LDLi+PItclt9zeYz3ZDFIgtmz5cg=
